...
all_seq = []
with open("../4.Analysis/4.2.motif/hs.CPT.delta.scores.regions.sigdelta.nls.fasta") as f:
	for line in f:
		line = line.strip('\n')
		if re.search("^>", line):
			continue
...
ctr_seq = []
with open("../4.Analysis/4.2.motif/hs.CPT.delta.scores.regions.sigdelta.ctr.fasta") as f:
	for line in f:
		line = line.strip('\n')
		if re.search("^>", line):
			continue
...
out = open("../5.plots/sigdelta.nls.aa.freq.txt", "w")
...
out = open("../5.plots/sigdelta.nls.aapair0.freq.txt", "w")
out.write("AA\tFreq\tFreqCtr\n")
for i in cor_seq:
	out.write(i+'\t'+str(cor_seq[i])+'\t'+str(cor_ctr[i])+'\n')
out.close()
...
for i in AA_list_k:
	for j in AA_list_k:
		cor_seq[i+j] = len(re.findall(r''+i+"."+j+'', all_seq))
		cor_ctr[i+j] = len(re.findall(r''+i+"."+j+'', ctr_seq))
out = open("../5.plots/sigdelta.nls.aapair1.freq.txt", "w")
out.write("AA\tFreq\tFreqCtr\n")
for i in cor_seq:
	out.write(i+'\t'+str(cor_seq[i])+'\t'+str(cor_ctr[i])+'\n')
out.close()
...
out = open("../5.plots/sigdelta.nls.aapair2.freq.txt", "w")
out.write("AA\tFreq\tFreqCtr\n")
for i in cor_seq:
	out.write(i+'\t'+str(cor_seq[i])+'\t'+str(cor_ctr[i])+'\n')
out.close()
