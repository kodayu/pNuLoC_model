...
out = open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.scores", "w")
...
pro2pos2trunc = {}
pro2pos2delta = {}
with open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.scores") as f:
	#f.readline()
	for line in f:
		data = line.strip('\n').split('\t')
...
out = open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores", "w")
with open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.scores") as f:
...
copyfile("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores", "../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores.tmp")
###########write a circle to find all regions
allvalues, alllines = [], []
finalscore = 0
with open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores") as f:
	for line in f:
		data = line.strip('\n').split('\t')
...
while sorted(allvalues, reverse = True)[0] > 0.1:
	...
	out = open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.scores.tmp", "w")
	for i in range(len(all_line)):
		out.write(all_line[i]+'\t'+str(score[i][1])+'\n')
	...
	pro2pos2trunc = {}
	pro2pos2delta = {}
	with open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.scores.tmp") as f:
		#f.readline()
		for line in f:
			data = line.strip('\n').split('\t')
			if data[0] not in pro2pos2score:
	...
	out = open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores.regions", "w")
	allvalues = []
	with open("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores.tmp") as f:
		for line in f:
			line = line.strip('\n')
			data = line.split('\t')
			if data[0] in pro2pos2trunc:
				if int(data[4]) in pro2pos2trunc[data[0]]:
					...
					out.write(line+'\n')
	out.close()
	copyfile("../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores.regions", "../4.Analysis/4.1.hsTruncated/allpros/"+sys.argv[1]+".CPT.delta.scores.tmp")
