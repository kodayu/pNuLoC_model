#!python
#hs.cpt.delta.method1.sigdelta.sites.regions
import sys
nls2site = {}
...
out = open("../4.Analysis/4.1.hsTruncated/"+sys.argv[1]+".4predict.seq", "w")
...
with open("../4.Analysis/4.1.hsTruncated/"+sys.argv[1]+".4predict.seq") as f:
	tittle = f.readline().strip('\n')
	for line in f:
		data = line.strip('\n').split('\t')
		if data[1] != "Nucleus":
			continue
		if len(data[7]) < max_len:
			all_line.append(line.strip('\n'))
			all_data.append(data[7])
...
out = open("../4.Analysis/4.1.hsTruncated/"+sys.argv[1]+".4predict.results", "w")
