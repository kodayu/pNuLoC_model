...
roc_file(t_score, t_label, num="pre_hs")

out = open("../4.Analysis/4.1.hsTruncated/human.dataset.scores", "w")
...
all_data, all_label, all_line = [], [], []
with open("../4.Analysis/4.1.hsTruncated/human.truncated.dataset") as f:
	tittle = f.readline().strip('\n')
	for line in f:
		data = line.strip('\n').split('\t')
		if len(data[2]) < max_len:
...
print(roc_auc_score(t_label,t_score))
roc_file(t_score, t_label, num="pre_hs_truncated")

out = open("../4.Analysis/4.1.hsTruncated/human.truncated.dataset.scores", "w")
out.write(tittle+'\tScore'+'\n')
for i in range(len(all_line)):
	out.write(all_line[i]+'\t'+str(score[i][1])+'\n')
