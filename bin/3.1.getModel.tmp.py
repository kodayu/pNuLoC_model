...
max_len = 2000
train_data, train_label = read_data("../0.datadeal/eukaryota.train.fixedlength8.dataset")
...
vali_data, vali_label = read_data("../0.datadeal/eukaryota.valid.fixedlength8.dataset")
...
test_data, test_label = read_data("../0.datadeal/eukaryota.test.fixedlength8.dataset")
...
from tensorflow.keras.layers import Dense, Dropout, Activation, Lambda, Dot, Layer
...
bi_rnn = Bidirectional(LSTM(do, return_sequences=True, kernel_regularizer=l2(0.01), recurrent_regularizer=l2(0.01), bias_regularizer=l2(0.01)))(act11)
drop = Dropout(dr)(bi_rnn)
atten = Attention(return_sequences=False)(drop)
# softmax classifier
...
history1 = model1.fit(train_ohe, y_train, batch_size=64, epochs=20, validation_data=(val_ohe, y_vali),callbacks=cb)
...
#roc_file(t_score, t_label, num="train")

score = model1.predict(val_ohe)
t_label = [int(i) for i in vali_label]
t_score = [i[1] for i in score]
print(roc_auc_score(t_label,t_score))
#roc_file(t_score, t_label, num="valid")

score = model1.predict(test_ohe)
t_label = [int(i) for i in test_label]
t_score = [i[1] for i in score]
print(roc_auc_score(t_label,t_score))
#roc_file(t_score, t_label, num="test")

model1.save_weights('../3.model/model.h5')
json = open('../3.model/model.json',"w")
json.write(model1.to_json())
json.close()

loss = history1.history["loss"]
np.savetxt('../3.model/model.loss.txt',loss)
valloss = history1.history["val_loss"]
np.savetxt('../3.model/model.valloss.txt',valloss)
