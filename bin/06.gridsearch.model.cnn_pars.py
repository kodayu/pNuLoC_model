...
if sys.argv[4] == "CNNBiLSTMAtten1":
	...
	cnnnodes = [32, 64, 128, 256]
	drate = [0.3, 0.4, 0.5, 0.6, 0.7]
	lstmnodes = [64, 128, 256, 512]
	for co in cnnnodes:
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					out1 = open("../2.rocs/"+sys.argv[1].split("fixedlength")[1]+"_"+sys.argv[4]+"_"+str(co)+"_"+str(dr)+"_"+str(do)+"_"+str(i)+".roc", "w")
					...
					out1.write(sys.argv[1].split("fixedlength")[1]+"\t"+sys.argv[4]+"\t"+str(co)+"\t"+str(dr)+"\t"+str(do)+"\t"+str(i)+'\t'+str(roc1)+'\t'+str(roc2)+'\t'+str(roc3)+'\n')
					out1.close()
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					...
					out1.write(sys.argv[1].split("fixedlength")[1]+"\t"+sys.argv[4]+"\t"+str(co)+"\t"+str(dr)+"\t"+str(do)+"\t"+str(i)+'\t'+str(roc1)+'\t'+str(roc2)+'\t'+str(roc3)+'\n')
					out1.close()
elif sys.argv[4] == "CNNBiLSTMAtten3":
	from tensorflow.keras.layers import Dense, Dropout, Activation
	from tensorflow.keras.layers import Masking, Embedding, Input, LSTM, Bidirectional, Conv1D
	from tensorflow.keras.layers import Conv1D, Concatenate, MaxPooling1D, BatchNormalization, Add, Flatten
	from tensorflow.keras.models import Model
	repeatime = int(sys.argv[5])
	cnnnodes = [32, 64, 128, 256]
	drate = [0.5]                      #[0.3, 0.4, 0.5, 0.6, 0.7]
	lstmnodes = [128]                        #[64, 128, 256, 512]
	for co in cnnnodes:
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					...
					out1.write(sys.argv[1].split("fixedlength")[1]+"\t"+sys.argv[4]+"\t"+str(co)+"\t"+str(dr)+"\t"+str(do)+"\t"+str(i)+'\t'+str(roc1)+'\t'+str(roc2)+'\t'+str(roc3)+'\n')
					out1.close()
elif sys.argv[4] == "CNNBiLSTMAtten4":
	from tensorflow.keras.layers import Dense, Dropout, Activation
	from tensorflow.keras.layers import Masking, Embedding, Input, LSTM, Bidirectional, Conv1D
	from tensorflow.keras.layers import Conv1D, Concatenate, MaxPooling1D, BatchNormalization, Add, Flatten
	from tensorflow.keras.models import Model
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					out1 = open("../2.rocs/"+sys.argv[1].split("fixedlength")[1]+"_"+sys.argv[4]+"_"+str(co)+"_"+str(dr)+"_"+str(do)+"_"+str(i)+".roc", "w")
					x_input = Input(shape=(2000,22))
					cnn1 = residual_block2(x_input, co, (4, 8, 12, 16))
					cnn2 = residual_block2(x_input, co, (8, 12, 4, 16))
					cnn3 = residual_block2(x_input, co, (12, 4, 16, 8))
					cnn4 = residual_block2(x_input, co, (16, 12, 8, 4))
					con1234 = Concatenate()([cnn1, cnn2, cnn3, cnn4])
					bn11 = BatchNormalization()(con1234)
					act11 = Activation('relu')(bn11)
					bi_rnn = Bidirectional(LSTM(do, kernel_regularizer=l2(0.01), recurrent_regularizer=l2(0.01), bias_regularizer=l2(0.01)))(act11)
