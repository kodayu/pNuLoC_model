		...
		for dr in drate:
			for do in densenodes:
				for i in range(repeatime):
					...
					roc_file(t_score, t_label, num="model_CNN")
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					...
					print(roc_auc_score(t_label,t_score))
					roc2 = roc_auc_score(t_label,t_score)
					roc_file(t_score, t_label, num="model_BiLSTM")
elif sys.argv[4] == "CNNBiLSTM":
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					...
					print(roc_auc_score(t_label,t_score))
					roc2 = roc_auc_score(t_label,t_score)
					roc_file(t_score, t_label, num="model_CNNBiLSTM")
elif sys.argv[4] == "CNNBiLSTMAtten":
	from tensorflow.keras.layers import Dense, Dropout, Activation
	from tensorflow.keras.layers import Masking, Embedding, Input, LSTM, Bidirectional, Conv1D
	from tensorflow.keras.layers import Conv1D, Concatenate, MaxPooling1D, BatchNormalization, Add, Flatten
	from tensorflow.keras.models import Model
		...
		for dr in drate:
			for do in lstmnodes:
				for i in range(repeatime):
					...
					print(roc_auc_score(t_label,t_score))
					roc2 = roc_auc_score(t_label,t_score)
					roc_file(t_score, t_label, num="model_CNNBiLSTMAtten")
else:
	print("---------------------------------Wrong Input---------------------------")
