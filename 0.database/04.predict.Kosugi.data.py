...
##human.dataset
all_data, all_line = [], []
with open("Kosugi_NLSactivity.txt") as f:
	tittle = f.readline().strip('\n')
	for line in f:
		data = line.strip('\n').split('\t')
		tmpres = data[2:15]
		tmpres = [i for i in tmpres if i != ""]
		if len(tmpres) < max_len:
			#all_data.append("".join(tmpres))
			all_data.append("".join(tmpres))
...
out = open("Kosugi_NLSactivity.score.txt", "w")
